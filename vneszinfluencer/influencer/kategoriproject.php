...
if(isset($_GET['id'])){
    ...
    if(isset($_POST['cari_project'])){
        ...
        $dataproject = mysqli_query($db, "SELECT * FROM tabel_project where nama_project LIKE '%".$_POST['cari_project']."%' AND kategori = '".$_GET['id']."' ORDER by id_project DESC limit $halaman_awal, $batas");
        $nomor = $halaman_awal+1;
       
        if($dataproject){
            header("Location: kategoriproject.php?id= ".$id);
            exit;
        }
    }else{
        $batas = 10;
        ...
        $dataproject = mysqli_query($db, "SELECT * FROM tabel_project where kategori = '".$_GET['id']."' ORDER by id_project DESC limit $halaman_awal, $batas");
        $nomor = $halaman_awal+1;
    }
...
<body>
    ...
    <div class="container-fluid" style="background-color: #009688;">
        <div class="row align-items-center py-1 px-xl-5">
            ...
            <div class="col-lg-6 col-6 text-left">
                <form action="kategoriproject.php?id=<?= $id;?>" method="post">
    ...
    <div class="container-fluid pt-3">
        <div class="row px-xl-5 pb-3">
            <?php if(mysqli_num_rows($dataproject) == 0) { ?>
                <div class="col-lg-12 text-center" style="margin-bottom: 100px; margin-top:100px">
                <h5><span class="px-2 text-muted">Project Belum Tersedia</span></h5>
                </div>
            <?php }?>
        ...
        <nav>
                                    <ul class="pagination mt-3">
                                        <li class="page-item">
                                            <a class="page-link" <?php if($halaman > 1){ echo "href='?id=$id&halaman=$previous'"; } ?>>Sebelumnya</a>
                                        </li>
                                        <?php 
                                        for($x=1;$x<=$total_halaman;$x++){
                                            ?> 
                                            <li class="page-item"><a class="page-link" href="?id=<?=$_GET['id'];?>&halaman=<?php echo $x ?>"><?php echo $x; ?></a></li>
                                            <?php
                                        }
                                        ?>				
                                        <li class="page-item">
                                            <a  class="page-link" <?php if($halaman < $total_halaman) { echo "href='?id=$id&halaman=$next'"; } ?>>Selanjutnya</a>
                                        </li>
                                    </ul>
                                </nav>
    </div>
