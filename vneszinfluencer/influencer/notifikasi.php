...
$pesan_status_hapus_data ='';
    ...
    if(isset($_SESSION['jumlah_notifikasi_influencer'])){
        unset($_SESSION['jumlah_notifikasi_influencer']);
    }
...
if(isset($_POST['cari_notifikasi'])){
    $cari_notifikasi = $_POST['cari_notifikasi'];
    $batas = 10;
	...
	$previous = $halaman - 1;
	$next = $halaman + 1;
    $result2 = mysqli_query($db, "SELECT * FROM tabel_notifikasi");
    $jumlah_data = mysqli_num_rows($result2);
	$total_halaman = ceil($jumlah_data / $batas);
    
    $notifikasi = mysqli_query($db, "SELECT * FROM tabel_notifikasi where waktu_notif LIKE '%".$_POST['cari_notifikasi']."%' AND nama_influencer='".$akun_influencer['nama_influencer']."' AND jenis_notif='notif_influencer' ORDER by waktu_notif DESC limit $halaman_awal, $batas");
    ...
    $notifikasi = mysqli_query($db, "SELECT * FROM tabel_notifikasi where nama_influencer='".$akun_influencer['nama_influencer']."' AND jenis_notif='notif_influencer' ORDER by waktu_notif DESC limit $halaman_awal, $batas");
    $nomor = $halaman_awal+1;
}
...
<head>
    <meta charset="utf-8">
    <title>VnesZBrand|Notifikasi</title>
            ...
            <div class="col-lg-6 col-6 text-left">
                <form action="notifikasi.php" method="post">
                    <div class="input-group">
                        <input type="text" class="form-control" placeholder="Cari Riwayat Transaksi..." name="cari_notifikasi" value="<?= $cari_notifikasi;?>">
    ...
    <div class="container-fluid pt-5 py-5">
        ...
        <div class="col-lg-12">
            ...
            <div class="text-center mb-4">
                <h3><span class="px-2">Notifikasi</span></h3>
            </div>
        ...
        <div class="col-lg-12 table-responsive mb-5">
                <table class="table table-bordered text-center mb-0">
                    ...
                    <tbody class="align-middle">
                        <?php while($data_notifikasi = mysqli_fetch_array($notifikasi)) { ?>
                        <tr>
                            <td class="align-middle"><?= $nomor; ?></td>
                            <td class="align-middle"><?= $data_notifikasi['waktu_notif']; ?></td>
                            <td class="align-middle"><?= $data_notifikasi['nama_brand']; ?></td>
                            <td class="text-justify"><?= $data_notifikasi['isi_notif']; ?></td>
                            <td class="align-middle">
                            <div class="input-group quantity mx-auto" style="width: 100px;">
                                    <div class="input-group-btn mx-auto">
                                        <a href="detailprogress.php?id=<?= $data_notifikasi['id_progress'];?>" class="btn btn-sm btn-success" title="Detail">
                                        <i class="fa fa-eye"></i>
                                        </a>
                                    </div>
                                </div>
