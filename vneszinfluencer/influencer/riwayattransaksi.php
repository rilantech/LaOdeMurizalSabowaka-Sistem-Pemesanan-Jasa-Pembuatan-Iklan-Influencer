...
$result3 = mysqli_query($db, "SELECT SUM(gaji_influencer) AS jumlah_pendapatan FROM tabel_progress_project where nama_influencer='".$akun_influencer['nama_influencer']."' AND status_project='Dibayar' ORDER by waktu_pembayaran DESC");
$row = mysqli_fetch_array($result3);
$jumlah_pendapatan = $row['jumlah_pendapatan'];

if(isset($_POST['cari_transaksi'])){
    $cari_transaksi = $_POST['cari_transaksi'];
    $batas = 10;
    ...
    $riwayattransaksi = mysqli_query($db, "SELECT * FROM tabel_progress_project where nama_project LIKE '%".$_POST['cari_transaksi']."%' AND nama_influencer='".$akun_influencer['nama_influencer']."' AND status_project='Dibayar' ORDER by waktu_pembayaran DESC limit $halaman_awal, $batas");
    ...
    $riwayattransaksi = mysqli_query($db, "SELECT * FROM tabel_progress_project where nama_influencer='".$akun_influencer['nama_influencer']."' AND status_project='Dibayar' ORDER by waktu_pembayaran DESC limit $halaman_awal, $batas");
    $nomor = $halaman_awal+1;
}
?>
<!DOCTYPE html>
...
<head>
    <meta charset="utf-8">
    <title>VnesZInfluencer|Riwayat Transaksi</title>
    ...
    <div class="container-fluid" style="background-color: #009688;">
        <div class="row align-items-center py-1 px-xl-5">
            ...
            <div class="col-lg-6 col-6 text-left">
                <form action="riwayattransaksi.php" method="post">
                    <div class="input-group">
                        <input type="text" class="form-control" placeholder="Cari Riwayat Transaksi..." name="cari_transaksi" value="<?= $cari_transaksi;?>">
    ...
    <div class="container-fluid pt-5 py-5">
        ...
        <div class="col-lg-12">
            ...
            <div class="text-center mb-4">
                <h3><span class="px-2">Riwayat Transaksi</span></h3>
            </div>
            <div class="container-fluid bg-secondary mb-5">
            <div class="d-flex flex-column align-items-center justify-content-center py-4">
                <h5 class="font-weight-semi-bold text-uppercase mb-3">Jumlah Pendapatan:</h5>
                <div class="d-inline-flex">
                    <p class="m-0">Rp <?= number_format($jumlah_pendapatan, 0, ',', '.'); ?></p>
                </div>
            </div>
            </div>
            <button class="btn btn-primary py-2 px-4 mb-3 rounded" type="submit" id="sendMessageButton" onclick="window.location='profil.php';"><i class="fa fa-arrow-left"></i> Kembali</button>
        </div>
        <div class="col-lg-12 table-responsive mb-5">
                <table class="table table-bordered text-center mb-0">
                    ...
                    <?php while($data_progress_project = mysqli_fetch_array($riwayattransaksi)) { ?>
                        <tr>
                            <td class="align-middle"><?= $nomor; ?></td>
                            <td class="align-middle"><?= $data_progress_project['waktu_pembayaran']; ?></td>
                            <td class="align-middle"><img src="../brand/gambar/<?= $data_progress_project['struk_bukti_pembayaran']; ?>" alt="" width="100" height="100"></td>
                            <td class="align-middle"><?= $data_progress_project['nama_project']; ?></td>
                            <td class="align-middle"><?= $data_progress_project['nama_brand']; ?></td>
                            <td class="align-middle">Rp <?= number_format($data_progress_project['gaji_influencer'], 0, ',', '.'); ?></td>
                            <td class="align-middle">
                            <div class="input-group quantity mx-auto" style="width: 100px;">
                                    <div class="input-group-btn mx-auto">
                                        <a href="detailriwayattransaksi.php?id=<?= $data_progress_project['id_progress'];?>" class="btn btn-sm btn-success" title="Detail">
                                        <i class="fa fa-eye"></i>
                                        </a>
                                    </div>
                                </div>
