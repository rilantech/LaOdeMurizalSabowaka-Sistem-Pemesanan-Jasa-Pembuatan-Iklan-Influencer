...
if(isset($_POST['cari_project'])){
    ...
    $result2 = mysqli_query($db, "SELECT * FROM tabel_project");
    $jumlah_data = mysqli_num_rows($result2);
    $total_halaman = ceil($jumlah_data / $batas);

    $dataproject = mysqli_query($db, "SELECT * FROM tabel_project where nama_project LIKE '%".$_POST['cari_project']."%' AND nama_brand='".$akun_brand['nama_brand']."' ORDER by id_project DESC limit $halaman_awal, $batas");
    ...
    $dataproject = mysqli_query($db, "SELECT * FROM tabel_project where nama_brand='".$akun_brand['nama_brand']."' ORDER by id_project DESC limit $halaman_awal, $batas");
    $nomor = $halaman_awal+1;
}
...
<head>
    <meta charset="utf-8">
    <title>VnesZBrand|Project</title>
    ...
    <div class="container-fluid pt-5 py-5">
        ...
        <div class="col-lg-12">
            ...
            <div class="text-center mb-4">
                <h3><span class="px-2">Daftar Project</span></h3>
            </div>
            <button class="btn btn-primary py-2 px-4 mb-3 rounded" type="submit" id="sendMessageButton" onclick="window.location='tambahproject.php';"><i class="fa fa-plus"></i>Tambah</button>
        </div>
        <div class="col-lg-12 table-responsive mb-5">
                <table class="table table-bordered text-center mb-0">
                    ...
                    <tbody class="align-middle">
                        <?php while($data_project = mysqli_fetch_array($dataproject)) { ?>
                        <tr>
                            <td class="align-middle"><?= $nomor; ?></td>
                            <td class="align-middle"><?= $data_project['nama_project']; ?></td>
                            <td class="align-middle"><?= $data_project['kategori']; ?></td>
                            <td class="align-middle"><?= $data_project['lama_kontrak']; ?></td>
                            <td class="align-middle" hidden>Rp <?= number_format($data_project['gaji_influencer'], 0, ',', '.'); ?></td>
                            <td class="align-middle">
                            <div class="input-group quantity mx-auto" style="width: 100px;">
                                    <div class="input-group-btn">
                                        <a href="editproject.php?id=<?= $data_project['id_project'];?>" class="btn btn-sm btn-warning" title="Edit">
                                        <i class="fa fa-edit"></i>
                                        </a>
                                    </div>
                                    <input type="text" class="form-control form-control-sm bg-white border-0 text-center">
                                    <div class="input-group-btn">
                                        <a href="project.php?id_hapus=<?=$data_project['id_project'];?>" class="btn btn-sm btn-danger" title="Hapus">
